...
module.exports = function () {
  return {
    ...
    APP_SYNC_URL: "https://api.prod.tranmere-web.com",
    APP_SYNC_OPTIONS: {
     headers: {
    ...
    pages: [],
    sendResponse: function(code, obj) {
      return {
    ...
    sendHTMLResponse: function(page, maxAge) {
      return {
        isBase64Encoded: false,
        headers: {
    ...
    extractExtraFromHTML: function(html, date, competition, season, match) {
      ...
      match.home = this.translateTeamName(match.home);
      match.visitor = this.translateTeamName(match.visitor);
      match.opposition = this.translateTeamName(match.opposition);
    ...
    extractSquadFromHTML: function(html, date, competition, season) {
      const $ = cheerio.load(html);
      ...
      scorersItems.each((i, el) => {
        ...
        for(var i=0; i < minute.length; i ++) {
            var goal = {
                ...
                Season: season,
                Scorer: this.translatePlayerName(text.match(minRegex)[0]),
                //Assist: null,
                GoalType: '',
                //AssistType: null,
                Minute: minute[i],
            ...
            goal.Opposition = this.translateTeamName(goal.Opposition);
      ...
      rows.each((i, el) => {
        ...
        if(i != 0 &&  i != 12 ) {
            ...
            var app = {
                ...
                Competition:  competition,
                Season: season,
                Name: this.translatePlayerName(text),
            ...
            app.opposition = this.translateTeamName(app.opposition);
            ...
            if(i < 12)
                ...
                for(var y=0; y < apps.length; y++) {
                    if(apps[y].SubTime == subMin && !apps[y].SubbedBy) {
                       ...
                       apps[y].SubbedBy = this.translatePlayerName(subName);
    ...
    extractMatchesFromHTML: function(html) {
      const $ = cheerio.load(html);
    ...
    translatePlayerName: function(input) {
    ...
    translateTeamName: function(input) {
    ...
    buildPageView: function (view, pageTpl, outputPath, noindex) {
       console.log("Building " + outputPath);
       if(outputPath != './web.out/index.html')
           view.url = outputPath.replace('./web.out/','/');
       ...
       if(view.carousel) {
           for(var i=0; i < view.carousel.length; i++) {
            var img = this.buildImagePath("screenshots/" + view.carousel[i].image, 1000, 400)
    ...
    findAllPlayers : async function() {
       var query = encodeURIComponent("{listTranmereWebPlayerTable(limit:300){items{name}}}");
       var results = await axios.get(`${this.APP_SYNC_URL}/graphql?query=${query}`, this.APP_SYNC_OPTIONS);
       var players = [];
    ...
    getAllCupCompetitions : async function(size) {
       var query = encodeURIComponent("{listTranmereWebCompetitions(limit:500){items{name}}}");
       var results = await axios.get(`${this.APP_SYNC_URL}/graphql?query=${query}`, this.APP_SYNC_OPTIONS);
    ...
    getTopScorersBySeason : async function() {
       ...
       for(var i= 1977; i <= this.getYear(); i++) {
           var result = await axios.get("https://api.prod.tranmere-web.com/player-search/?season="+i+"&sort=Goals", this.apiOptions);
    ...
    findAllTeams : async function(size) {
       var query = encodeURIComponent("{listTranmereWebClubs(limit:500){items{name}}}");  
       var result = await axios.get(`${this.APP_SYNC_URL}/graphql?query=${query}`, this.APP_SYNC_OPTIONS);
