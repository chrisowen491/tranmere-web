import { MatchEventAdmin } from "@/components/apps/admin/MatchEventAdmin";
...
export const metadata: Metadata = {
  title: "Manage match events | Admin",
  description: "Manage additional match events in the Tranmere-Web database.",
};
...
export default async function MatchEventAdminPage({
  searchParams,
}: {
  searchParams: Promise<{ season?: string; date?: string }>;
}) {
  await requireAdminPage("/admin/match-events");
      ...
      <header className="border-b border-white/10 bg-[#071a2b] text-white">
        <div className="mx-auto max-w-7xl px-6 py-14 sm:px-10 lg:px-12 lg:py-20">
          <div className="flex flex-wrap gap-5 text-xs font-bold uppercase tracking-[0.14em]">
            ...
            <span className="text-blue-300">Match events</span>
          </div>
          <h1 className="mt-5 font-display text-5xl font-semibold tracking-[-0.04em] sm:text-6xl">
            Match event archive
          </h1>
          <p className="mt-4 max-w-2xl text-base leading-7 text-white/65">
            Record penalty saves, penalty misses, own goals and future event
            types against a match and player.
          </p>
        </div>
      </header>
