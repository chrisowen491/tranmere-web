...
import { findAllTranmereManagers } from "@/lib/apiFunctions";
import { Metadata } from 'next';
export const runtime = "edge";


export const metadata: Metadata = {
  title: 'Tranmere Rovers Managerial Records - Tranmere-Web',
  description: 'Records of all Tranmere Rovers managers',
};

export default async function ManagerRecords() {
  
  const managers = await findAllTranmereManagers();
              ...
              <div className="row separated">
                ...
                <article className="col-md-8 content-body">
                  ...
                  {managers.map((manager) => (
                    <div className="card stacked" style={{marginBottom:"50px"}} key={manager.name}>
                      <div className="card-body">
                          <div className="row gutter-2 align-items-center">
                              <div className="col">
                                  <h3 className="card-title mb-1" itemProp="name">{manager.name}</h3>
                                  <p className="card-text mb-2"><small>
                                      From
                                      <time dateTime={manager.dateJoined} className="text-muted d-block">{manager.dateJoined}</time>
                                  </small></p>
                                  <p className="card-text mb-2"><small>
                                      To
                                      <time dateTime={manager.dateLeft} className="text-muted d-block">{manager.dateLeft}</time>
                                  </small></p>
                              </div>
                          </div>
