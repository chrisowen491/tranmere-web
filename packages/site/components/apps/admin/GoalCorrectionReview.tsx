...
export function GoalCorrectionReview({
  initialCorrections,
}: {
  initialCorrections: GoalCorrection[];
}) {
  ...
  async function review(
    ...
    try {
      ...
      setMessage(`Goal correction ${status}.`);
    } catch (error) {
      setMessage(
  ...
  if (!corrections.length)
    return (
      <p className="border border-[#071a2b]/15 bg-[#fffdf8] p-10 text-center">
        No goal corrections are awaiting review.
      </p>
    );
        ...
        >
          <header className="bg-[#071a2b] p-6 text-white sm:p-8">
            ...
            <h2 className="mt-2 font-display text-3xl font-semibold">
              {display(correction.current.scorer)} v {correction.opposition}
            </h2>
            <Link
          ...
          <div className="p-6 sm:p-8">
            <div className="space-y-4">
              {Object.entries(correction.changes).map(([field, proposed]) => {
                const key = field as keyof EditableGoal;
                return (
                  <div
                    key={field}
                    className="grid gap-3 border-b border-[#071a2b]/10 pb-4 sm:grid-cols-[150px_minmax(0,1fr)_40px_minmax(0,1fr)]"
                  >
                    <p className="text-xs font-bold uppercase tracking-[0.12em] text-[#071a2b]/50">
                      {editableGoalLabels[key]}
                    </p>
                    <div>
