...
export function PlayerAppsTable(props: { title: string, records: Appearance[]}) {
              ...
              <tbody className="divide-y divide-gray-200 text-sm">
                {props.records.map((record, idx) => (
                  <tr key={idx}>
                    ...
                    <td className="whitespace-nowrap px-3 py-4">{record.Competition}</td>
                    <td className="whitespace-nowrap px-3 py-4">
                        <LinkButton text={`${record.Opposition}`} href={`/match/${record.Season}/${record.Date}`} ></LinkButton>
                    </td>
                    <td className="whitespace-nowrap hidden px-3 py-3.5 lg:table-cell">{record.Type}</td>
