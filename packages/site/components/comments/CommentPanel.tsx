...
export default function CommentPanel(props: {
  ...
  const onSubmit = async (formData: FormData) => {
    ...
    try {
      const data = await fetch("/api/comment", {
        method: "POST",
        body: JSON.stringify({ text: formData.get("comment") as string, rating: formData.get("rating") as string, url: props.url  }),
        headers: {
  ...
  return (
    <div className="mt-5 dark:text-gray-50 text-gray-900">
        ...
        <div className="col-span-full">
          ...
          <div className="col-span-full mt-4">
            ...
            <div className="mt-2"> 
            <select name="rating" defaultValue={rating} id="rating"  className="block w-full rounded-md border-0 py-1.5shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:max-w-xs sm:text-sm sm:leading-6">
                  ...
                  <>
                    <div key={idx} className="flex space-x-4 text-sm">
                      <div className="flex-none py-10">
                        <img alt="" src={comment.user.picture} className="h-10 w-10 rounded-full bg-gray-100" />
                      </div>
                      <div className={classNames(idx === 0 ? '' : 'border-t border-gray-200', 'flex-1 py-10')}>
                        <h3 className="font-medium">{ comment.user.name}</h3>
                        <p>
                          <time dateTime={comment.created_at.toString()}>{comment.created_at}</time>
                        </p>
