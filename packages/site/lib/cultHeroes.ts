...
function isCupCompetition(competition: string | null) {
  return Boolean(
    competition &&
    (/cup|trophy|play.?off/i.test(competition) ||
      /^(FA|LC)\s*\d/i.test(competition)) &&
...
export function buildCultHeroSections(
  ...
  longestAbsences: PlayerMilestoneRow[],
  goals: GoalRow[],
  profiles: Map<string, PlayerStatisticsProfile>,
): CultHeroSection[] {
  ...
  const longSpells = [...summariesByPlayer.entries()]
    ...
    .sort(
      (a, b) =>
        b.value - a.value ||
        b.detail.localeCompare(a.detail) ||
        a.name.localeCompare(b.name),
    )
    .slice(0, 8);

  const comebacks = longestAbsences
    .filter(
      (milestone) =>
        isPlayerName(milestone.player_name) &&
        milestone.milestone_value !== null &&
        milestone.milestone_value >= 365,
    )
    .map((hero) => {
      const years = Math.round(hero.milestone_value! / 365.25);
      return {
        name: hero.player_name,
        headline: `Returned after ${years} ${years === 1 ? "year" : "years"} away`,
        detail: `Back in a Rovers line-up on ${hero.match_date}.`,
        value: years,
  ...
  const promotionSeasons = new Map(
    HONOURS_SEASONS.filter((entry) =>
      entry.achievements.some(
        (achievement) =>
          achievement.kind === "Promotion" || achievement.kind === "Play-offs",
      ),
    ).map((entry) => [entry.season, entry.achievements]),
  );
  const promotionContributions = new Map<
    string,
    { appearances: number; seasons: number[] }
  >();
  for (const row of summaryRows) {
    const season = Number(row.season);
    if (!promotionSeasons.has(season) || !isPlayerName(row.player_name))
      continue;
  ...
  return [
    {
      ...
      description:
        "The longest uninterrupted runs recorded in the Tranmere archive.",
    ...
    },
    {
      ...
      description:
        "The biggest gaps between one Rovers appearance and the next.",
    ...
    },
    {
      ...
      description:
        "The players most regularly trusted to make an impact from the bench.",
    ...
    },
    {
      ...
      description:
        "The most-used players across Rovers promotion and play-off-winning campaigns.",
      heroes: addProfiles(promotionHeroes, profiles),
    },
  ];
}

export async function getCultHeroSections(
  db: D1Database,
): Promise<CultHeroSection[]> {
  ...
  const names = [
    ...new Set([
      ...summaryRows.map((row) => row.player_name),
      ...longestAbsences.map((milestone) => milestone.player_name),
      ...goals.map((goal) => goal.scorer),
    ]),
  ];
  const profiles = await getPlayerStatisticsProfiles(db, names);
  return buildCultHeroSections(summaryRows, longestAbsences, goals, profiles);
}
