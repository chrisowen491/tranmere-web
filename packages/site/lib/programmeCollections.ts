...
export async function getCollectionEntry(
  ...
  return db
    .prepare(
      `SELECT auth_sub, game_id, status, condition_notes, purchase_notes,
              created_at, updated_at
       FROM ProgrammeCollections WHERE auth_sub = ? AND game_id = ?`,
    )
    .bind(authSub, gameId)
  ...
  await db
    .prepare(
      `INSERT INTO ProgrammeCollections (
         auth_sub, game_id, status, condition_notes, purchase_notes, created_at, updated_at
       ) VALUES (?, ?, ?, ?, ?, ?, ?)
       ON CONFLICT(auth_sub, game_id) DO UPDATE SET
         status = excluded.status,
         condition_notes = excluded.condition_notes,
         purchase_notes = excluded.purchase_notes,
         updated_at = excluded.updated_at`,
    )
    .bind(authSub, gameId, status, conditionNotes, purchaseNotes, now, now)
    .run();
  return getCollectionEntry(db, authSub, gameId);
}

const collectionSelect = `
  SELECT pc.auth_sub, pc.game_id, pc.status, pc.condition_notes, pc.purchase_notes,
...
export async function getUserCollection(db: D1Database, authSub: string) {
  const result = await db
    .prepare(
      `${collectionSelect}
       WHERE pc.auth_sub = ? AND g.no_programme_issued = 0
...
export async function updateCollectionVisibility(
  ...
  await db
    ...
    .bind(publicId, visible ? 1 : 0, 0, authSub)
    .run();
  return publicId;
}
...
export async function getPublicCollection(db: D1Database, publicId: string) {
  ...
  const result = await db
    .prepare(
      `${collectionSelect}
       WHERE pc.auth_sub = ? AND pc.status IN ('wanted', 'trade')
         AND g.no_programme_issued = 0
       ORDER BY g.match_date DESC, pc.game_id ASC`,
    )
    .bind(profile.auth_sub)
    .all<ProgrammeCollectionDisplay>();
