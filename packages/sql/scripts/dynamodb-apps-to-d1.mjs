...
function parseApp(line, source) {
  ...
  const substitutedBy = optionalString(item, 'SubbedBy', source);
  const substituteSubstitutedBy = optionalString(item, 'SubSubbedBy', source);
  return {
    ...
    opposition: canonicalizeClubName(
      requiredString(item, 'Opposition', source)
    ),
    ...
    substitutedByShirtNumber:
      season < 1986 && substitutedBy
        ? 12
        : optionalInteger(item, 'SubbedByNumber', source),
    substituteSubstitutedByShirtNumber:
      season < 1986 && substituteSubstitutedBy
        ? 12
        : optionalInteger(item, 'SubSubbedByNumber', source)
  };
}
...
function values(app) {
  return `(${[
    ...
    app.substitutedByShirtNumber ?? 'NULL',
    app.substituteSubstitutedByShirtNumber ?? 'NULL'
  ].join(', ')})`;
}
...
for (let index = 0; index < apps.length; index += rowsPerStatement) {
  ...
  substituted_by_shirt_number, substitute_substituted_by_shirt_number
) VALUES
...
ON CONFLICT(id) DO UPDATE SET
  ...
  substituted_by_shirt_number = excluded.substituted_by_shirt_number,
  substitute_substituted_by_shirt_number = excluded.substitute_substituted_by_shirt_number;`);
}
