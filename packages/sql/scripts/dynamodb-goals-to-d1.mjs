...
function parseGoal(record, source) {
  ...
  const sixYardBox = flag(item, '6YardBox', source);
  const eighteenYardBox = flag(item, '18YardBox', source);
  const longRange = flag(item, 'LongRange', source);
